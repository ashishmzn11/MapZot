...
// Options for Step 9: Marketing & Ads
const marketingPriorityOptions = [
  { key: "A", label: "Get custom geofencing and location-based targeting." },
  { key: "B", label: "Create custom Audience by real-world visitation." },
  { key: "C", label: "Get Higher ROAS and more conversions" },
  { key: "D", label: "Better OOH / Billboards / Ad Placements" },
  { key: "E", label: "Understand true trade area for loyal audience" },
  { key: "F", label: "Other" },
];

// Assuming this component handles the priorities for this specific persona
export default function Step9EPriorities() {
  const navigate = useNavigate();
  // Placeholder context properties for this specific Step 9 (e.g., suffixed with 'Marketing')
  const { 
    marketingPriorities9, 
    setMarketingPriorities9, 
    marketingPriorities9Other, 
    setMarketingPriorities9Other 
  } = useContext(AppContext);

  // ------------ Load existing data ------------
  let savedOtherText = marketingPriorities9Other || "";
  let savedKeys = [];

  // Convert saved labels to keys for initial state
  if (Array.isArray(marketingPriorities9)) {
    savedKeys = marketingPriorities9
      .map((item) => {
        const found = marketingPriorityOptions.find((op) => op.label === item);
        return found ? found.key : null;
      })
      .filter(Boolean);

    // If "Other" text exists in context but "F" wasn't explicitly saved, add "F"
  ...
  const handleSave = () => {
    ...
    let finalLabels = selectedKeys
      .filter((k) => k !== "F") // Exclude the 'F' key itself
      .map((k) => marketingPriorityOptions.find((op) => op.key === k).label);
    ...
    if (selectedKeys.includes("F")) {
      ...
      setMarketingPriorities9Other(otherText.trim());
      // Add the custom text to the final labels list
      finalLabels.push(otherText.trim());
    } else {
      setMarketingPriorities9Other(""); // Clear other text if "Other" isn't selected
    }

    // Save the final list of labels in context
    setMarketingPriorities9(finalLabels);
